...
#+BEGIN_SRC sh :results output :tangle no
  (
      cd $OGP_EXPORT_DIR
      if [ -d .git ]; then
          LC_MESSAGES=en git --no-pager log -1 HEAD --date=short --pretty=format:'File under git version control - commit <a href=\"url/commit/%H\">%h</a> - %ad' \
...
:PROPERTIES:
:MKDIRP:   yes
:TANGLE:   (concat this-directory "doc/pdf/default-class.sty")
:END:
**** Basics
#+BEGIN_SRC latex
  \NeedsTeXFormat{LaTeX2e}
  \ProvidesPackage{default-class}[2014/10/25 v0.01 Bundling a default LaTeX class for org-pages]
#+END_SRC
